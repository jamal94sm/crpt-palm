"""
main.py — MaskFeat pretraining on the same palmprint dataset/config family
as crpt-palm's proposed method. Verified against open-mmlab/mmpretrain's
MaskFeat implementation (models, optimizer, schedule) and the MaskFeat
paper (Wei et al., CVPR 2022) for HOG parameters and masking ratio.
"""
...
from models import MaskedViT, MaskFeatHead, FeatureExtractor
from hog import HOGGenerator
from masking import batch_block_mask
...
def resolve_output_path(cfg):
    if getattr(cfg, "output_name", None):
        ...
        name = f"maskfeat_{cfg.mode}_multiseed_n{getattr(cfg, 'n_runs', 3)}_seed{cfg.seed}.txt"
    else:
        name = f"maskfeat_{cfg.mode}_seed{cfg.seed}.txt"
...
def build_no_decay_param_groups(model, wd):
    """Official: bias_decay_mult=0, norm_decay_mult=0, flat_decay_mult=0,
    mask_token decay_mult=0 -- any 1D param (bias, LayerNorm gain/bias)
    and mask_token get NO weight decay; everything else uses wd."""
    decay, no_decay = [], []
...
def train_maskfeat(cfg, train_loader, eval_dict, out_path):
    img_size = (cfg.img_size, cfg.img_size)
    print(f"\n Building MaskFeat encoder...")
    encoder = MaskedViT(img_size, cfg.num_patches, cfg.embed_dim).to(cfg.device)

    hog_out_dim = cfg.hog_nbins * (cfg.hog_pool and (cfg.img_size // cfg.hog_pool // cfg.num_patches) ** 2 or 1) * 3
    grid_hc = cfg.img_size // cfg.hog_pool
    cells_per_patch = grid_hc // cfg.num_patches
    hog_out_dim = cfg.hog_nbins * (cells_per_patch ** 2) * 3

    head = MaskFeatHead(cfg.embed_dim, hog_out_dim).to(cfg.device)
    hog_gen = HOGGenerator(nbins=cfg.hog_nbins, pool=cfg.hog_pool,
                           gaussian_window=cfg.hog_gaussian_window).to(cfg.device)

    n_enc = sum(p.numel() for p in encoder.parameters())
    n_head = sum(p.numel() for p in head.parameters())
    print(f" Encoder: {n_enc/1e6:.2f}M | Head: {n_head/1e6:.3f}M params | "
          f"HOG out_dim={hog_out_dim} (cells/patch={cells_per_patch}x{cells_per_patch})")
    print(f" Mask ratio: {cfg.mask_ratio}  (block-wise, official-style)")

    param_groups = build_no_decay_param_groups(encoder, cfg.adamw_wd) + \
                   build_no_decay_param_groups(head, cfg.adamw_wd)
    ...
    def adjust_lr(step):
        """Official: LinearLR warmup (start_factor=1e-6) then
        CosineAnnealingLR to eta_min=1e-6 -- reproduced with the same
        shape, scaled to this project's --epochs instead of a fixed 300."""
        if step < warmup_steps:
            lr = base_lr * (1e-6 + (1 - 1e-6) * step / max(1, warmup_steps))
        else:
            s = step - warmup_steps
            m = total_steps - warmup_steps
            q = 0.5 * (1 + math.cos(math.pi * s / max(1, m)))
            eta_min = 1e-6
            lr = eta_min + (base_lr - eta_min) * q
    ...
    print(f"\n{'─'*70}\n Training MaskFeat ({total_steps} steps)\n{'─'*70}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for images, labels in train_loader:
            ...
            mask = batch_block_mask(B, cfg.num_patches, cfg.mask_ratio,
                                    cfg.min_mask_block, device=cfg.device)

            z = encoder(images, mask=mask)
            pred = head(z)

            with torch.no_grad():
                target = hog_gen(images, cfg.num_patches)

            mask_flat = mask.unsqueeze(-1).float()
            loss = F.mse_loss(pred * mask_flat, target * mask_flat, reduction="sum") / \
                   (mask_flat.sum() * pred.size(-1) + 1e-6)
    ...
    append_text(out_path, f"\nRESULTS -- method=maskfeat mode={cfg.mode} "
...
def run_multi_seed(cfg, out_path):
    ...
    for i in range(n_runs):
        ...
        final_entry = train_maskfeat(run_cfg, train_loader, eval_dict, out_path)
    ...
    def _print_summary():
        print(f"\n{'='*80}\n MULTI-SEED SUMMARY ({n_runs} runs, maskfeat, "
              f"CI level={level:.0%})\n{'='*80}")
...
def main():
    ...
    train_maskfeat(cfg, train_loader, eval_dict, out_path)
